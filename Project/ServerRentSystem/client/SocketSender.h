...
public:
   ...
   ~SocketSender();
   virtual void sendLog(list<MLogRec>& logs) throw(ClientException);    // 通过网络发送日志记录

private:
   void connectServer(void) throw(SocketException);                     // 连接服务器
   void readFailFile(list<MLogRec>& logs) throw(ReadException);         // 读取上次失败的日志记录
   void sendData(list<MLogRec>& logs) throw(SendException);             // 通过网络发送日志记录给服务器
   void saveFailFile(list<MLogRec>& log) throw(SaveException);          // 保存发送失败的日志记录
