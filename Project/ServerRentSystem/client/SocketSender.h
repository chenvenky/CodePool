...
{
public:
   SocketSender(std::string const& failFile, std::string const& ip, const short port, int sockfd);
   ~SocketSender();
   virtual void sendLog(list<MLogRec>& logs);   // 通过网络发送日志记录

private:
   void connectSever(void);                     // 连接服务器
   ...
   std::string m_failFile;      // 上次发送失败的已匹配记录的文件
   std::string m_ip;            // 服务器进程 IP 
   std::string m_port;          // 服务器进程端口
   int sockfd;                  // 套接字描述符
